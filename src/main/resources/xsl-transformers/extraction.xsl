...
<xsl:stylesheet
	...
	exclude-result-prefixes="xs xd dc rdfs swrl owl2xml owl xsd swrlb rdf f dcterms"
	...
	<xsl:variable name="default-labels"
		select="document(concat($def-lang,'.xml'))" />
	<xsl:variable name="labels"
		select="document(concat($lang,'.xml'))" />
	<xsl:variable name="possible-ontology-urls"
		select="($ontology-url,concat($ontology-url,'/'),concat($ontology-url,'#'))"
		as="xs:string+" />
	<xsl:variable name="mime-types"
		select="('jpg','image/jpg','jpeg','image/jpg','png','image/png')"
		as="xs:string+" />

	<xsl:variable name="prefixes-uris" as="xs:string*">
		<xsl:variable name="declared-prefixes"
			select="in-scope-prefixes($rdf)" as="xs:string*" />
		...
		<xsl:variable name="non-declared-uris" as="xs:string*">
			<xsl:variable name="temp-non-declared" as="xs:string*">
				<xsl:for-each select="$existing-uri">
					<xsl:variable name="index"
						...
						as="xs:integer?" />
					<xsl:if
						test="exists($index) and substring(.,$index + 1) != ''">
						<xsl:variable name="ns"
							select="substring(.,1,$index)" as="xs:string?" />
			...
			<xsl:sequence
				select="distinct-values($temp-non-declared)" />
		</xsl:variable>

		<xsl:variable name="non-declared-prefixes"
			as="xs:string*">
			<xsl:for-each select="$non-declared-uris">
				<xsl:variable name="string"
					select="substring(.,1,string-length(.) - 1)" as="xs:string" />
				<xsl:variable name="index"
					select="f:string-last-index-of($string,'/')" as="xs:integer?" />
				<xsl:variable name="selected"
					select="if ($index) then substring($string,$index + 1) else $string"
					as="xs:string" />
				<xsl:sequence
					select="lower-case(replace($selected,'\.|_| |:','-'))" />
			</xsl:for-each>
		</xsl:variable>

		<xsl:variable name="all-prefixes"
			select="($declared-prefixes,$non-declared-prefixes)" as="xs:string*" />
		<xsl:variable name="all-uris"
			select="($declared-uris,$non-declared-uris)" as="xs:string*" />
	...
	<xsl:template name="structure">
		<xsl:variable name="titles"
			select="dc:title|dcterms:title" as="element()*" />
		<head>
			<xsl:choose>
	...
	<xsl:template
		match="dc:description[f:isInLanguage(.)][normalize-space() != '']"
		mode="ontology">
		<h2 id="introduction">
			<xsl:value-of
				select="f:getDescriptionLabel('introduction')" />
		</h2>
	...
	<xsl:template match="dc:description[@*:resource]"
		...
		<xsl:variable name="index"
			select="f:string-last-index-of($url,'\.')" as="xs:integer?" />
			...
			<object data="{@*:resource}">
				<xsl:if test="$extension != ''">
					<xsl:variable name="mime"
						select="$mime-types[index-of($mime-types,$extension) + 1]"
						as="xs:string?" />
					<xsl:if test="$mime != ''">
	...
	<xsl:template
		match="dc:rights[f:isInLanguage(.)]|dcterms:rights[ancestor::owl:Ontology][f:isInLanguage(.)]">
		<div class="copyright">
			<xsl:call-template name="get.content" />
		</div>
	</xsl:template>
	...
	<xsl:template
		match="dc:title[f:isInLanguage(.)] | dcterms:title[f:isInLanguage(.)]"
		mode="ontology">
		<h1>
	...
	<xsl:template
		match="dc:date|dcterms:date[ancestor::owl:Ontology]">
		<dt>
			<xsl:value-of select="f:getDescriptionLabel('date')" />
			:
		</dt>
		<dd>
			<xsl:choose>
				<xsl:when test="matches(.,'\d\d\d\d-\d\d-\d\d')">
					...
					<xsl:value-of
						select="$tokens[3],$tokens[2],$tokens[1]" separator="/" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:apply-templates />
				</xsl:otherwise>
	...
	<xsl:template match="owl:versionInfo[f:isInLanguage(.)]">
		<dt>
			<xsl:value-of
				select="f:getDescriptionLabel('currentversion')" />
			:
		</dt>
		<dd>
			<xsl:apply-templates />
	...
	<xsl:template
		match="owl:priorVersion | owl:backwardCompatibleWith | owl:incompatibleWith">
		...
		<dt>
			<xsl:value-of
				select="f:getDescriptionLabel($versionLabel)" />
			:
		</dt>
		<dd>
			<xsl:choose>
		...
		<div id="{generate-id()}" class="entity">
			<xsl:call-template name="get.entity.name">
				...
				<xsl:with-param name="toc.string"
					select="f:getDescriptionLabel('classtoc')" tunnel="yes"
					as="xs:string" />
			</xsl:call-template>
	...
	<xsl:template match="owl:NamedIndividual">
		<div id="{generate-id()}" class="entity">
			<xsl:call-template name="get.entity.name">
				...
				<xsl:with-param name="toc.string"
					select="f:getDescriptionLabel('namedindividualtoc')" tunnel="yes"
					as="xs:string" />
			</xsl:call-template>
			<xsl:call-template name="get.entity.metadata" />
			<xsl:apply-templates select="rdfs:comment" />
			<xsl:call-template
				name="get.individual.description" />
			<xsl:apply-templates
				select="dc:description[normalize-space() != ''] , dc:description[@*:resource]" />
		</div>
	</xsl:template>
	...
	<xsl:template
		match="@*:about | @*:resource | @*:ID | @*:datatype">
	...
	<xsl:template
		match="owl:Class[not(parent::rdf:RDF)] | rdfs:Datatype[not(parent::rdf:RDF)] | owl:DataRange[not(parent::rdf:RDF)]">
		<xsl:apply-templates />
	</xsl:template>
		...
		<xsl:for-each select="element()">
			<xsl:apply-templates select=".">
				<xsl:with-param name="type"
					select="'namedindividual'" tunnel="yes" />
			</xsl:apply-templates>
			<xsl:if test="position() != last()">
				<xsl:text> , </xsl:text>
			</xsl:if>
	...
	<xsl:template match="rdf:rest">
		<xsl:if
			test="rdf:Description[rdf:type/@*:resource = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#List'] | rdf:List">
			<xsl:text> , </xsl:text>
		</xsl:if>
		<xsl:apply-templates
	...
	<xsl:template
		...
		<div id="{generate-id()}" class="entity">
			...
			<p>
				<xsl:for-each
					select="owl:members/rdf:Description/(@*:about|@*:ID)">
					<xsl:apply-templates select=".">
						<xsl:with-param name="type" select="'class'"
							as="xs:string" tunnel="yes" />
					</xsl:apply-templates>
		...
		<div id="{generate-id()}" class="entity">
			...
			<p>
				...
				<strong>
					<xsl:text> </xsl:text>
					<xsl:value-of
						select="f:getDescriptionLabel('issubclassof')" />
				</strong>
			</p>
		...
		<div id="{generate-id()}" class="entity">
			...
			<p>
				...
				<strong>
					<xsl:text> </xsl:text>
					<xsl:value-of
						select="f:getDescriptionLabel('isequivalentto')" />
				</strong>
			</p>
			<p style="text-align:right">
				<xsl:apply-templates
					select="owl:equivalentClass">
					<xsl:with-param name="list" select="false()"
						tunnel="yes" />
				</xsl:apply-templates>
			</p>
	...
	<xsl:template
		...
		<div id="{generate-id()}" class="entity">
			...
			<p>
				<xsl:apply-templates
					select="element()[not(self::rdfs:subClassOf)]" />
				<strong>
					<xsl:text> </xsl:text>
					<xsl:value-of
						select="f:getDescriptionLabel('issubclassof')" />
	...
	<xsl:template
		...
		<div id="{generate-id()}" class="entity">
			...
			<p>
				<xsl:apply-templates
					select="element()[not(self::owl:equivalentClass)]" />
				<strong>
					<xsl:text> </xsl:text>
					<xsl:value-of
						select="f:getDescriptionLabel('isequivalentto')" />
	...
	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#FunctionalProperty']">
		<xsl:value-of
			select="f:getDescriptionLabel('functional')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#InverseFunctionalProperty']">
		<xsl:value-of
			select="f:getDescriptionLabel('inversefunctional')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#ReflexiveProperty']">
		<xsl:value-of select="f:getDescriptionLabel('reflexive')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#IrreflexiveProperty']">
		<xsl:value-of
			select="f:getDescriptionLabel('irreflexive')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#SymmetricProperty']">
		<xsl:value-of select="f:getDescriptionLabel('symmetric')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#AsymmetricProperty']">
		<xsl:value-of
			select="f:getDescriptionLabel('asymmetric')" />
	</xsl:template>

	<xsl:template
		match="rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#TransitiveProperty']">
		<xsl:value-of
			select="f:getDescriptionLabel('transitive')" />
	</xsl:template>

	<xsl:template match="owl:hasValue">
		<xsl:call-template name="get.cardinality.formula">
			<xsl:with-param name="type" select="'namedindividual'"
				tunnel="yes" />
			<xsl:with-param name="op" select="'value'"
				as="xs:string" />
		</xsl:call-template>
	</xsl:template>

	<xsl:template
		match="owl:cardinality | owl:qualifiedCardinality">
		<xsl:call-template name="get.cardinality.formula">
			<xsl:with-param name="op" select="'exactly'"
				as="xs:string" />
		</xsl:call-template>
	</xsl:template>

	<xsl:template
		match="owl:maxCardinality | owl:maxQualifiedCardinality">
		<xsl:call-template name="get.cardinality.formula">
	...
	<xsl:template
		match="owl:minCardinality | owl:minQualifiedCardinality">
		<xsl:call-template name="get.cardinality.formula">
	...
	<xsl:template match="owl:onClass">
		...
		<xsl:apply-templates select="@*:resource">
			<xsl:with-param name="type" as="xs:string"
				tunnel="yes" select="'class'" />
		</xsl:apply-templates>
	</xsl:template>

	<xsl:template match="owl:onProperty">
		<xsl:apply-templates
			select="@*:resource|rdf:Description/owl:inverseOf">
			<xsl:with-param name="type" as="xs:string"
				tunnel="yes" select="'property'" />
		</xsl:apply-templates>
	</xsl:template>

	<xsl:template
		match="owl:allValuesFrom | owl:someValuesFrom">
		<xsl:variable name="logic"
			select="if (self::owl:allValuesFrom) then 'only' else 'some'"
			as="xs:string" />
		<xsl:text> </xsl:text>
		<span class="logic">
	...
	<xsl:template name="get.original.source">
		<xsl:if test="exists(rdfs:isDefinedBy)">
			<dl class="definedBy">
				<dt>
					<xsl:value-of
						select="f:getDescriptionLabel('isdefinedby')" />
				</dt>
	...
	<xsl:template name="get.class.description">
		<xsl:if
			test="exists(rdfs:subClassOf | owl:hasKey) or f:hasDisjoints(.) or f:hasMembers(.) or f:hasSubclasses(.) or f:isInDomain(.) or f:isInRange(.) or f:hasEquivalent(.) or f:hasSameAs(.) or f:hasPunning(.)">
	...
	<xsl:template name="get.individual.description">
		...
		<xsl:if
			...
			<dl class="description">
				...
				<xsl:call-template
					name="get.individual.assertions">
					<xsl:with-param name="type"
						select="'namedindividual'" tunnel="yes" />
				</xsl:call-template>
				<xsl:call-template name="get.entity.punning" />
	...
	<xsl:template name="get.entity.type">
		<xsl:if test="exists(rdf:type)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('belongsto')" />
			</dt>
			<xsl:apply-templates select="rdf:type">
				<xsl:with-param name="type" tunnel="yes"
					select="'class'" as="xs:string" />
			</xsl:apply-templates>
		</xsl:if>
	</xsl:template>

	<xsl:template name="get.entity.sameas">
		<xsl:variable name="currentSameAs"
			select="f:getSameAs(.)" as="attribute()*" />
		<xsl:if test="exists($currentSameAs)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('issameas')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.entity.punning">
		<xsl:variable name="iri" select="@*:about|@*:ID"
			as="xs:string" />
		<xsl:variable name="type" select="f:getType(.)"
			as="xs:string" />
		...
		<xsl:if test="$punningsequence">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('isalsodefinedas')" />
			</dt>
			<dd>
	...
	<xsl:function name="f:checkPunning" as="xs:boolean">
		...
		<xsl:variable name="iri" select="$el/@*:about|$el/@*:ID"
			as="xs:string" />
		<xsl:variable name="type" select="f:getType($el)"
			as="xs:string" />
	...
	<xsl:template name="get.individual.assertions">
			...
			<assertions>
				<xsl:for-each select="element()">
					<xsl:variable name="currentURI"
						select="concat(namespace-uri(.),local-name(.))" as="xs:string" />
					<xsl:if
						test="some $prop in (/rdf:RDF/(owl:ObjectProperty|owl:DatatypeProperty)/(@*:about|@*:ID)) satisfies $prop = $currentURI">
		...
		<xsl:if test="$assertions//@*:about">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('individualassertions')" />
			</dt>
	...
	<xsl:template name="get.entity.disjoint">
		<xsl:variable name="currentDisjoints"
			select="f:getDisjoints(.)" as="attribute()*" />
		<xsl:if test="exists($currentDisjoints)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('isdisjointwith')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.class.superclasses">
		<xsl:if test="exists(rdfs:subClassOf)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hassuperclasses')" />
			</dt>
			<xsl:apply-templates select="rdfs:subClassOf" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="get.class.subclasses">
		...
		<xsl:variable name="sub-classes" as="attribute()*"
			select="/rdf:RDF/owl:Class[some $res in rdfs:subClassOf/@*:resource satisfies $res = $about]/(@*:about|@*:ID)" />
		<xsl:if test="exists($sub-classes)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hassubclasses')" />
			</dt>
			<dd>
				<xsl:for-each select="$sub-classes">
					<xsl:sort select="f:getLabel(.)" data-type="text"
						order="ascending" />
					<xsl:apply-templates select="." />
					<xsl:if test="position() != last()">
						<xsl:text>, </xsl:text>
	...
	<xsl:template name="get.class.indomain">
		...
		<xsl:if test="exists($properties)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('isindomainof')" />
			</dt>
			<dd>
				<xsl:for-each select="$properties">
					<xsl:sort select="f:getLabel(.)" order="ascending"
						data-type="text" />
					<xsl:apply-templates select=".">
						<xsl:with-param name="type" as="xs:string"
							tunnel="yes"
	...
	<xsl:template name="get.class.inrange">
		...
		<xsl:if test="exists($properties)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('isinrangeof')" />
			</dt>
			<dd>
				<xsl:for-each select="$properties">
					<xsl:sort select="f:getLabel(.)" order="ascending"
	...
	<xsl:template name="get.class.members">
		...
		<xsl:variable name="members" as="attribute()*"
			select="/rdf:RDF/owl:NamedIndividual[some $res in rdf:type/@*:resource satisfies $res = $about]/(@*:about|@*:ID)" />
		<xsl:if test="exists($members)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hasmembers')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.property.description">
		<xsl:if
			test="exists(rdfs:subPropertyOf | rdfs:domain | rdfs:range | owl:propertyChainAxiom) or f:hasSubproperties(.) or f:hasInverseOf(.) or f:hasDisjoints(.) or f:hasEquivalent(.) or f:hasSameAs(.) or f:hasPunning(.)">
			<div class="description">
				<xsl:call-template name="get.characteristics" />
				<dl>
					<xsl:call-template
						name="get.property.equivalentproperty" />
					<xsl:call-template
						name="get.property.superproperty" />
					<xsl:call-template
						name="get.property.subproperty" />
	...
	<xsl:template name="get.property.inverse">
		<xsl:variable name="currentInverseOf"
			select="f:getInverseOf(.)" as="attribute()*" />
		<xsl:if test="exists($currentInverseOf)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('isinverseof')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.property.chain">
		<xsl:if test="exists(owl:propertyChainAxiom)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hassubpropertychains')" />
			</dt>
			<xsl:apply-templates
				select="owl:propertyChainAxiom" />
		</xsl:if>
	</xsl:template>

	<xsl:template name="get.property.equivalentproperty">
		<xsl:variable name="currentEquivalent"
			select="f:getEquivalent(.)" as="attribute()*" />
		<xsl:if test="exists($currentEquivalent)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hasequivalentproperties')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.property.superproperty">
		<xsl:if test="exists(rdfs:subPropertyOf)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hassuperproperties')" />
			</dt>
	...
	<xsl:template name="get.property.subproperty">
		<xsl:variable name="type"
			select="if (self::owl:AnnotationProperty) then 'annotation' else 'property'"
			as="xs:string" />
		<xsl:variable name="about" select="(@*:about|@*:ID)"
			as="xs:string" />
		<xsl:variable name="sub-properties" as="attribute()*"
			select="/rdf:RDF/(if ($type = 'property') then owl:DatatypeProperty | owl:ObjectProperty else owl:AnnotationProperty)[some $res in rdfs:subPropertyOf/@*:resource satisfies $res = $about]/(@*:about|@*:ID)" />
		<xsl:if test="exists($sub-properties)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hassubproperties')" />
			</dt>
			<dd>
	...
	<xsl:template name="get.property.domain">
		<xsl:if test="exists(rdfs:domain)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hasdomain')" />
			</dt>
	...
	<xsl:template name="get.property.range">
		<xsl:if test="exists(rdfs:range)">
			<dt>
				<xsl:value-of
					select="f:getDescriptionLabel('hasrange')" />
			</dt>
	...
	<xsl:template name="get.ontology.url">
		<xsl:if
			test="exists((@*:about|@*:ID)[normalize-space() != ''])">
			<dl>
				<dt>IRI:</dt>
				<dd>
	...
	<xsl:template name="get.version">
		<xsl:if
			test="exists(owl:versionInfo | owl:priorVersion | owl:backwardCompatibleWith | owl:incompatibleWith | dc:date | dcterms:date)">
			<dl>
				<xsl:apply-templates
					select="dc:date | dcterms:date" />
				<xsl:apply-templates select="owl:versionInfo" />
				<xsl:apply-templates select="owl:priorVersion" />
				<xsl:apply-templates
					select="owl:backwardCompatibleWith" />
				<xsl:apply-templates
					select="owl:incompatibleWith" />
			</dl>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="get.imports">
		<xsl:if test="exists(owl:imports)">
			<dl>
				<dt>
					<xsl:value-of
						select="f:getDescriptionLabel('importedontologies')" />
					:
				</dt>
	...
	<xsl:template name="get.entity.name">
		<xsl:variable name="url" select="@*:about|@*:ID"
			as="xs:string" />
		<a name="{$url}" />
		<xsl:if
			test="starts-with($url, if (ends-with($ontology-url,'#')) then $ontology-url else concat($ontology-url, '#'))">
	...
	<xsl:template name="get.author">
		<xsl:if
			test="exists(dc:creator | dc:contributor | dcterms:creator[ancestor::owl:Ontology] | dcterms:contributor[ancestor::owl:Ontology])">
			<dl>
				<xsl:if
					test="exists(dc:creator|dcterms:creator[ancestor::owl:Ontology])">
					<dt>
						<xsl:value-of
							select="f:getDescriptionLabel('authors')" />
						:
					</dt>
					<xsl:apply-templates
						select="dc:creator|dcterms:creator[ancestor::owl:Ontology]">
						<xsl:sort select="text()|@*:resource" data-type="text"
							order="ascending" />
					</xsl:apply-templates>
				</xsl:if>
				<xsl:if
					test="exists(dc:contributor|dcterms:contributor[ancestor::owl:Ontology])">
					<dt>
						<xsl:value-of
							select="f:getDescriptionLabel('contributors')" />
						:
					</dt>
					<xsl:apply-templates
						select="dc:contributor|dcterms:contributor[ancestor::owl:Ontology]">
						<xsl:sort select="text()|@*:resource" data-type="text"
							order="ascending" />
					</xsl:apply-templates>
				</xsl:if>
	...
	<xsl:template name="get.publisher">
		<xsl:if test="exists(dc:publisher | dcterms:publisher)">
			<dl>
				<dt>
					<xsl:value-of
						select="f:getDescriptionLabel('publisher')" />
					:
				</dt>
				<xsl:apply-templates
					select="dc:publisher|dcterms:publisher">
					<xsl:sort select="text()|@*:resource" data-type="text"
						order="ascending" />
				</xsl:apply-templates>
			</dl>
	...
	<xsl:template name="get.toc">
		<div id="toc">
			...
			<ol>
				<xsl:if
					test="exists(//owl:Ontology/dc:description[normalize-space() != ''])">
					<li>
						<a href="#introduction">
							<xsl:value-of
								select="f:getDescriptionLabel('introduction')" />
				...
				<xsl:if test="exists(//owl:ObjectProperty/element())">
					<li>
						<a href="#objectproperties">
							<xsl:value-of
								select="f:getDescriptionLabel('objectproperties')" />
						</a>
					</li>
				</xsl:if>
				<xsl:if test="exists(//owl:DatatypeProperty/element())">
					<li>
						<a href="#dataproperties">
							<xsl:value-of
								select="f:getDescriptionLabel('dataproperties')" />
						</a>
					</li>
				</xsl:if>
				<xsl:if test="exists(//owl:NamedIndividual/element())">
					<li>
						<a href="#namedindividuals">
							<xsl:value-of
								select="f:getDescriptionLabel('namedindividuals')" />
						</a>
					</li>
				</xsl:if>
				<xsl:if test="exists(//owl:AnnotationProperty)">
					<li>
						<a href="#annotationproperties">
							<xsl:value-of
								select="f:getDescriptionLabel('annotationproperties')" />
						</a>
					</li>
				</xsl:if>
				<xsl:if
					test="exists(//rdf:Description[exists(rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#AllDisjointClasses'])]) or exists(/rdf:RDF/(owl:Class|owl:Restriction)[empty(@*:about | @*:ID) and exists(rdfs:subClassOf|owl:equivalentClass)])">
					<li>
						<a href="#generalaxioms">
							<xsl:value-of
								select="f:getDescriptionLabel('generalaxioms')" />
						</a>
					</li>
				</xsl:if>
				<xsl:if
					test="exists(/rdf:RDF/(swrl:Imp | rdf:Description[rdf:type[@*:resource = 'http://www.w3.org/2003/11/swrl#Imp']]))">
					<li>
						<a href="#swrlrules">
							<xsl:value-of
								select="f:getDescriptionLabel('rules')" />
						</a>
					</li>
				</xsl:if>
				<li>
					<a href="#namespacedeclarations">
						<xsl:value-of
							select="f:getDescriptionLabel('namespaces')" />
					</a>
				</li>
			</ol>
		</div>
	...
	<xsl:template name="get.generalaxioms">
		<xsl:if
			test="exists(/rdf:RDF/rdf:Description[exists(rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#AllDisjointClasses'])]) or exists(/rdf:RDF/(owl:Class|owl:Restriction)[empty(@*:ID | @*:about) and exists(rdfs:subClassOf|owl:equivalentClass)])">
			<div id="generalaxioms">
				...
				<xsl:apply-templates
					select="/rdf:RDF/(rdf:Description[exists(rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#AllDisjointClasses'])]|(owl:Class|owl:Restriction)[empty(@*:ID | @*:about) and exists(rdfs:subClassOf|owl:equivalentClass)])" />
			</div>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="get.namespacedeclarations">
		<div id="namespacedeclarations">
			...
			<dl>
				<xsl:for-each
					select="distinct-values($prefixes-uris[position() mod 2 = 1])">
					...
					<xsl:if test=". != 'xml'">
						<dt>
							<xsl:choose>
								<xsl:when test="$prefix = ''">
									<em>
										<xsl:value-of
											select="f:getDescriptionLabel('namespace')" />
									</em>
								</xsl:when>
								<xsl:otherwise>
						...
						<dd>
							<xsl:value-of
								select="$prefixes-uris[index-of($prefixes-uris,$prefix)[1] + 1]" />
						</dd>
					</xsl:if>
				</xsl:for-each>
			</dl>
	...
	<xsl:template name="get.namedindividuals">
		<xsl:if test="exists(//owl:NamedIndividual/element())">
			<div id="namedindividuals">
				...
				<xsl:apply-templates
					select="/rdf:RDF/owl:NamedIndividual[exists(element())]">
					<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
						order="ascending" data-type="text" />
					<xsl:with-param name="type" tunnel="yes"
						as="xs:string" select="'individual'" />
				</xsl:apply-templates>
			</div>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="get.namedindividuals.toc">
		<ul class="hlist">
			<xsl:apply-templates
				select="/rdf:RDF/owl:NamedIndividual[exists(element())]" mode="toc">
				<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
					order="ascending" data-type="text" />
				<xsl:with-param name="type" tunnel="yes"
					as="xs:string" select="'individual'" />
	...
	<xsl:template name="get.objectproperties">
		<xsl:if test="exists(//owl:ObjectProperty/element())">
			<div id="objectproperties">
				...
				<xsl:apply-templates
					select="/rdf:RDF/owl:ObjectProperty[exists(element())]">
					<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
						order="ascending" data-type="text" />
					<xsl:with-param name="type" tunnel="yes"
						as="xs:string" select="'property'" />
				</xsl:apply-templates>
			</div>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="get.objectproperties.toc">
		<ul class="hlist">
			<xsl:apply-templates
				select="/rdf:RDF/owl:ObjectProperty[exists(element())]" mode="toc">
				<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
					order="ascending" data-type="text" />
				<xsl:with-param name="type" tunnel="yes"
	...
	<xsl:template name="get.annotationproperties">
		<xsl:if test="exists(//owl:AnnotationProperty)">
			<div id="annotationproperties">
				...
				<xsl:call-template
					name="get.annotationproperties.toc" />
				<xsl:apply-templates
					select="/rdf:RDF/owl:AnnotationProperty">
					<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
						order="ascending" data-type="text" />
					<xsl:with-param name="type" tunnel="yes"
						as="xs:string" select="'annotation'" />
	...
	<xsl:template name="get.annotationproperties.toc">
		<ul class="hlist">
			<xsl:apply-templates
				select="/rdf:RDF/owl:AnnotationProperty" mode="toc">
				<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
					order="ascending" data-type="text" />
				<xsl:with-param name="type" tunnel="yes"
					as="xs:string" select="'property'" />
	...
	<xsl:template name="get.dataproperties">
		<xsl:if test="exists(//owl:DatatypeProperty/element())">
			<div id="dataproperties">
				...
				<xsl:apply-templates
					select="/rdf:RDF/owl:DatatypeProperty[exists(element())]">
					<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
						order="ascending" data-type="text" />
					<xsl:with-param name="type" tunnel="yes"
						as="xs:string" select="'property'" />
	...
	<xsl:template name="get.dataproperties.toc">
		<ul class="hlist">
			<xsl:apply-templates
				select="/rdf:RDF/owl:DatatypeProperty[exists(element())]" mode="toc">
				<xsl:sort select="lower-case(f:getLabel(@*:about|@*:ID))"
					order="ascending" data-type="text" />
				<xsl:with-param name="type" tunnel="yes"
					as="xs:string" select="'property'" />
	...
	<xsl:template name="get.entity.type.descriptor">
		...
		<xsl:choose>
			...
			<xsl:when
				...
				<sup title="{f:getDescriptionLabel('objectproperty')}"
					class="type-op">op</sup>
			</xsl:when>
			<xsl:when
				test="($type = '' or $type = 'property') and $el[self::owl:DatatypeProperty]">
				<sup title="{f:getDescriptionLabel('dataproperty')}"
					class="type-dp">dp</sup>
			</xsl:when>
			<xsl:when
				test="($type = '' or $type = 'annotation') and $el[self::owl:AnnotationProperty]">
				<sup title="{f:getDescriptionLabel('annotationproperty')}"
					class="type-ap">ap</sup>
			</xsl:when>
			<xsl:when
				test="($type = '' or $type = 'individual') and $el[self::owl:NamedIndividual]">
				<sup title="{f:getDescriptionLabel('namedindividual')}"
					class="type-ni">ni</sup>
			</xsl:when>
		</xsl:choose>
	</xsl:template>

	<xsl:template name="get.backlink">
		<xsl:param name="toc" select="''" as="xs:string*"
			tunnel="yes" />
		<xsl:param name="toc.string" select="''" as="xs:string*"
			tunnel="yes" />
		<span class="backlink">
			<xsl:text> </xsl:text>
	...
	<xsl:template name="get.characteristics">
		...
		<xsl:if test="exists($nodes)">
			<p>
				<strong>
					<xsl:value-of
						select="f:getDescriptionLabel('hascharacteristics')" />
					:
				</strong>
				<xsl:text> </xsl:text>
	...
	<xsl:function name="f:isInLanguage" as="xs:boolean">
		<xsl:param name="el" as="element()" />
		<xsl:variable name="isRightLang"
			select="$el/@xml:lang = $lang" as="xs:boolean" />
		<xsl:variable name="isDefLang"
	...
	<xsl:function name="f:getPrefixFromIRI" as="xs:string?">
		...
		<xsl:if test="not(starts-with($iri,'_:'))">
			...
			<xsl:variable name="ns"
				select="if (contains($iriNew,'#')) then substring($iriNew,1,f:string-first-index-of($iriNew,'#')) else substring($iriNew,1,f:string-last-index-of(replace($iriNew,'://','---'),'/'))"
				as="xs:string" />

			<xsl:variable name="index"
				select="index-of($prefixes-uris,$ns)[1]" as="xs:integer?" />
	...
	<xsl:function name="f:hasSubproperties" as="xs:boolean">
		<xsl:param name="el" as="element()" />
		<xsl:variable name="type"
			select="if ($el/self::owl:AnnotationProperty) then 'annotation' else 'property'"
			as="xs:string" />
		<xsl:value-of
			select="exists($rdf/(if ($type = 'property') then owl:DatatypeProperty | owl:ObjectProperty else owl:AnnotationProperty)[some $res in rdfs:subPropertyOf/@*:resource satisfies $res = $el/(@*:about|@*:ID)])" />
	</xsl:function>

	<xsl:function name="f:getType" as="xs:string?">
		<xsl:param name="element" as="element()" />
		<xsl:variable name="type" select="local-name($element)"
			as="xs:string" />
		<xsl:choose>
			<xsl:when test="$type = 'Class'">
			...
			<xsl:when test="$type = 'ObjectProperty'">
				<xsl:value-of
					select="f:getDescriptionLabel('objectproperty')" />
			</xsl:when>
			<xsl:when test="$type = 'DatatypeProperty'">
				<xsl:value-of
					select="f:getDescriptionLabel('dataproperty')" />
			</xsl:when>
			<xsl:when test="$type = 'AnnotationProperty'">
				<xsl:value-of
					select="f:getDescriptionLabel('annotationproperty')" />
			</xsl:when>
			<xsl:when test="$type = 'DataRange'">
				<xsl:value-of
					select="f:getDescriptionLabel('datarange')" />
			</xsl:when>
			<xsl:when test="$type = 'NamedIndividual'">
				<xsl:value-of
					select="f:getDescriptionLabel('namedindividual')" />
			</xsl:when>
		</xsl:choose>
	</xsl:function>
	...
	<xsl:function name="f:getDescriptionLabel" as="xs:string">
		...
		<xsl:variable name="labelname"
			select="lower-case(replace($inputlabel,' +',''))" as="xs:string" />
	...
	<xsl:function name="f:hasPunning" as="xs:boolean">
		<xsl:param name="el" as="element()" />
		<xsl:variable name="iri" select="$el/(@*:about|@*:ID)"
			as="xs:string" />
		<xsl:variable name="type" select="f:getType($el)"
			as="xs:string" />
		<xsl:value-of
			select="exists($rdf/element()[@*:about = $iri or @*:ID = $iri][f:getType(.) != $type])" />
	</xsl:function>
</xsl:stylesheet>
